...
grid = np.arange(-2.0,1.0,0.0005).reshape(-1,1)
kT = 2.4943389
files = ["smd1/colvar", "smd2/colvar", "smd3/colvar", "smd4/colvar", "smd5/colvar"]
smd = []

krr = KernelRidge(alpha=0.1,gamma=10000.0,kernel='rbf')

# smooth interpolation of each individual realization
for ifile in files:
    X = np.loadtxt(ifile, skiprows=1, usecols=7).reshape(-1,1)
    y = np.loadtxt(ifile, skiprows=1, usecols=8).reshape(-1,1)
    krr.fit(X,y)
...
nn = MLPRegressor(hidden_layer_sizes=(48), activation='tanh', solver='adam', alpha=1e-5, early_stopping=False, max_iter=10000, random_state=57451, tol=1e-9)
nn.fit(X_train,y_train)
print(nn.score(X_test,y_test))

grid = np.arange(-2.0,1.01,0.01).reshape(-1,1)
